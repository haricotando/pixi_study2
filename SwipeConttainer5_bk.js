...
export class SwipeContainer5 extends PIXI.Container {
    ...
    onTouchMove(event){
        ...
        this.shadow.diffY = event.data.global.y - this.shadow.tapStartY;

        this.textFldD.text = `diffY: ${this.shadow.diffY}`;
    ...
    onTickerHandler(){
        ...
        */
        const numOfSnaps = this.cardList.length - 1;
        ...
        const currentCard = this.cardList[this.currentSnapId].card;
        if(this.shadow.diffY < -200){
            this.isCardDragging = true;
            currentCard.y = this.shadow.diffY;
            const shadowPerIndex = this.shadow.maxX / (dataProvider.deck.length-1);
            let selectedDest = Math.round(currentCard.index * shadowPerIndex);
            // this.shadow.destX = this.isCardSelected ? selectedDest : tmp;
        }else{
            this.isCardDragging = false;
        }

        if(this.shadow.diffY < -300){
            currentCard.selected.visible = true;
        }else{
            currentCard.selected.visible = false;
        }
